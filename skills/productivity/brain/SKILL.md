...
1. Before answering product/market/pipeline/strategy/quality questions, run:
   `python "$HERMES_HOME/scripts/brain_read.py" --sections INDEX,PRODUCTS,MARKET,BUYERS,PIPELINES,DECISIONS,PRINCIPLES,PR_QUALITY`
...
3. Persist durable learnings with:
   `python "$HERMES_HOME/scripts/brain_write.py" PRODUCTS --append --text "..."`
...
6. After editing PRINCIPLES or PR_QUALITY, run `python "$HERMES_HOME/scripts/sync_quality_skill.py"` (brain consolidate also does this).
